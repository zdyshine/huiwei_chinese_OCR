...
def model(images, weight_decay=1e-5, is_training=True):
    ...
    with tf.variable_scope('feature_fusion', values=[end_points.values]):
        ...
        with slim.arg_scope([slim.conv2d],
            ...
            f_score_32 = slim.conv2d(g[3], 1, 1, activation_fn=tf.nn.sigmoid, normalizer_fn=None)
            ...
            f_score_64 = slim.conv2d(g[2], 1, 1, activation_fn=tf.nn.sigmoid, normalizer_fn=None)
            # 4 channel of axis aligned bbox and 1 channel rotation angle
            geo_map = slim.conv2d(g[2], 4, 1, activation_fn=tf.nn.sigmoid, normalizer_fn=None) * FLAGS.text_scale
            # geo_map = slim.conv2d(g[3], 4, 1, activation_fn=tf.nn.sigmoid, normalizer_fn=None)
...
def loss(y_true_cls, y_pred_cls,
    ...
    print('y_pred_cls[0]:',y_pred_cls[0].shape)
    print('y_pred_cls[1]:',y_pred_cls[1].shape)
    print('training_mask:',training_mask.shape)
    # classification_loss = dice_coefficient(y_true_cls, y_pred_cls, training_mask)
    classification_loss_0 = dice_coefficient(y_true_cls, y_pred_cls[0], training_mask)
    classification_loss_1 = dice_coefficient(y_true_cls, y_pred_cls[1], training_mask)
    # scale classification loss to match the iou loss part
    classification_loss = classification_loss_0 + classification_loss_1
    classification_loss *= 0.01

    # d1 -> top, d2->right, d3->bottom, d4->left
    d1_gt, d2_gt, d3_gt, d4_gt, theta_gt = tf.split(value=y_true_geo, num_or_size_splits=5, axis=3)
    d1_pred_0, d2_pred_0, d3_pred_0, d4_pred_0, theta_pred_0 = tf.split(value=y_pred_geo[0], num_or_size_splits=5, axis=3)
    d1_pred_1, d2_pred_1, d3_pred_1, d4_pred_1, theta_pred_1 = tf.split(value=y_pred_geo[1], num_or_size_splits=5, axis=3)

    d1_pred = (d1_pred_0 + d1_pred_1)*0.5
    d2_pred = (d2_pred_0 + d2_pred_1)*0.5
    d3_pred = (d3_pred_0 + d3_pred_1)*0.5
    d4_pred = (d4_pred_0 + d4_pred_1)*0.5
    theta_pred = (theta_pred_0 + theta_pred_1)*0.5
