...
parser.add_argument('--s_trainroot', required=True, help='path to dataset')
parser.add_argument('--s_valroot', required=True, help='path to dataset')
parser.add_argument('--m_trainroot', required=True, help='path to dataset')
parser.add_argument('--m_valroot', required=True, help='path to dataset')
parser.add_argument('--l_trainroot', required=True, help='path to dataset')
parser.add_argument('--l_valroot', required=True, help='path to dataset')
...
if __name__ == '__main__':
    ...
    # # read train set
    # train_dataset = dataset.lmdbDataset(root=opt.trainroot,transform=train_tf)
    ...
    #     root=opt.valroot, transform=dataset.resizeNormalize((320, 32))) # 固长度的预测和测试


    # read s_train set
    s_train_dataset = dataset.lmdbDataset(root=opt.s_trainroot)
    ...
    # read s_train set
    m_train_dataset = dataset.lmdbDataset(root=opt.m_trainroot)
    ...
    # read l_train set
    l_train_dataset = dataset.lmdbDataset(root=opt.l_trainroot)
    ...
    s_test_dataset = dataset.lmdbDataset(
        root=opt.s_valroot, transform=dataset.resizeNormalize((320, 32)))

    m_test_dataset = dataset.lmdbDataset(
        root=opt.m_valroot, transform=dataset.resizeNormalize((512, 32)))

    l_test_dataset = dataset.lmdbDataset(
        root=opt.l_valroot, transform=dataset.resizeNormalize((768, 32)))
