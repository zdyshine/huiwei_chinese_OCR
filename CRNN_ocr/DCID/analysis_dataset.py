...
# 提取对应数据集 暴力循环，耗时，你看着改改
with codecs.open('train_data.txt') as f:
	with codecs.open('m_train.txt') as f1:
		lines = f.readlines()
		lines1 = f1.readlines()
		with codecs.open('m_train_label.txt','a') as f2:
			for line in lines:
				image = line.replace('\n','').split(' ')[0].split('/')[-1]
				# print(image)
				for l in lines1:
...
# tr_img_path = base_dir + 'l_train/'
# or_img_path = 'crnn_image/'
# imagelist = os.listdir(or_img_path)
# with codecs.open('m_train.txt','a') as f:
...
#获取对应长度图片/并移出到指定文件夹
# or_img_path = 'crnn_image/'
# for image in os.listdir(or_img_path):
#     h,w = Image.open(os.path.join(or_img_path, image)).size
#     if h < 210:
#         shutil.move(or_img_path+image, tr_img_path+image)
...
#         h,w = Image.open(os.path.join(img_path, image)).size
#         if h > 509:
#             filename.append(image)
#             # print(h)
#             H.append(h)
#             W.append(w)
# Train_size('crnn_image/')
# train_size = pd.DataFrame(columns=['name'])
# train_size['name'] = filename
# train_size['Hsize'] = H
# train_size['Wsize'] = W
# #程序所在文件
# train_size.to_csv('l_image_size.csv', header=True, index=None, encoding='utf_8_sig')
...
#def stati_image_size(image_dir, save_dir, big_w_dir): #（原图片路径，保存路径，大图路径）
...
##    print(image_hw_ratio_dict)
#
#    with open(os.path.join(save_dir, 'image_hw_ratio_dict.json'), 'w',encoding = 'utf-8') as f:
#        f.write(json.dumps(image_hw_ratio_dict, indent=4))
##    print(int(max(h_count_dict.keys()))) 
#        
#
##    x = range(int(max(h_count_dict.keys()))+1)##########这一步的数据有疑问#########
##    y = [0 for _ in x]
###    print(h_count_dict[h])
##    for h in sorted(h_count_dict.keys()):
##        print(h)
##        print('图片长度:{:f}~{:f}，有{:f}张图'.format(10*h, 10*h+10, h_count_dict[h])) 
###        y[h] = h_count_dict[h]
##    plot.plot_multi_line([x], [y], ['Length'], save_path='../../data/length.png', show=True)
##
##    x = range(int(max(w_count_dict.keys()))+1)
##    y = [0 for _ in x]
##    for w in sorted(w_count_dict.keys()):
##        print('图片宽度:{:f}~{:f}，有{:f}张图'.format(10*w, 10*w+10, w_count_dict[w])) 
###        y[w] = w_count_dict[w]
##    plot.plot_multi_line([x], [y], ['Width'], save_path='../../data/width.png', show=True)
##
##    x = range(int(max(r_count_dict.keys()))+1)
##    y = [0 for _ in x]
##    for r in sorted(r_count_dict.keys()):
##        print('图片比例:{:f}~{:f}，有{:f}张图'.format(8*r, 8*r+8, r_count_dict[r])) 
###        y[r] = r_count_dict[r]
##    x = [8*(_+1) for _ in x]
